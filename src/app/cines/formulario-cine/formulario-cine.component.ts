...
export class FormularioCineComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.modelo !== undefined){
      //modifica el form con el modelo recibido
      this.form.patchValue(this.modelo);
      this.coordenadaInicial.push({latitud: this.modelo.latitud, longitud: this.modelo.longitud});
      console.log("mostrar coordenadas.");
    }
  }

  coordenadaSeleccionada(coordenada: Coordenada){
    //Le paso la coordenada recibida al formulario.
     this.form.patchValue(coordenada);
  }

  OnSubmit(){
    //Para envíar los datos del formulario
